...
{
	TCHAR szPropertyInf[MAX_PATH] = {_T("\0")};
	...
	if(uTextStyle & DT_VCENTER)
	{
		tstrVAlgin = _T("center");
	}
	...
	if(!tstrVAlgin.empty())
	{
		pNode->SetAttribute("valign", StringConvertor::WideToUtf8(tstrVAlgin.c_str()));
	}
	...
	//rhaa
	if(pLabelUI->GetTextRenderingHintAntiAlias()!=0)
	{
		_stprintf_s(szPropertyInf, _T("%d"),pLabelUI->GetTextRenderingHintAntiAlias());
	...
	{
		_stprintf_s(szPropertyInf, _T("%d"),pLabelUI->GetGradientAngle());
		pNode->SetAttribute("gradientangle", StringConvertor::WideToUtf8(szPropertyInf));
	}
	//gradientlength
	if(pLabelUI->GetGradientLength()!=0)
	{
		_stprintf_s(szPropertyInf, _T("%d"),pLabelUI->GetGradientLength());
		pNode->SetAttribute("gradientlength", StringConvertor::WideToUtf8(szPropertyInf));
	}
	//transstroke 字体描边的颜色透明度 0-255
	if(pLabelUI->GetTransStroke()!=255)
	{
		_stprintf_s(szPropertyInf, _T("%d"),pLabelUI->GetTransStroke());
		pNode->SetAttribute("transstroke", StringConvertor::WideToUtf8(szPropertyInf));
	}
	//strokecolor 字体描边的颜色
	...
	if(pLabelUI->GetTextColor1()!=0xffffffff)
	{
		...
		_stprintf_s(szPropertyInf, _T("#%02X%02X%02X%02X"), HIBYTE(HIWORD(dwColor)), static_cast<BYTE>(GetBValue(dwColor)), static_cast<BYTE>(GetGValue(dwColor)), static_cast<BYTE>(GetRValue(dwColor)));
		pNode->SetAttribute("textcolor1", StringConvertor::WideToUtf8(szPropertyInf));
	}
	//textshadowcolora 字体阴影渐变色
	if(pLabelUI->GetTextShadowColorA()!=0xff000000)
	{
		DWORD dwColor = pLabelUI->GetTextShadowColorA();
		_stprintf_s(szPropertyInf, _T("#%02X%02X%02X%02X"), HIBYTE(HIWORD(dwColor)), static_cast<BYTE>(GetBValue(dwColor)), static_cast<BYTE>(GetGValue(dwColor)), static_cast<BYTE>(GetRValue(dwColor)));
		pNode->SetAttribute("textshadowcolora", StringConvertor::WideToUtf8(szPropertyInf));
	}
	//textshadowcolorb
	if(pLabelUI->GetTextShadowColorB()!=0xff000000)
	{
		DWORD dwColor = pLabelUI->GetTextShadowColorB();
		_stprintf_s(szPropertyInf, _T("#%02X%02X%02X%02X"), HIBYTE(HIWORD(dwColor)), static_cast<BYTE>(GetBValue(dwColor)), static_cast<BYTE>(GetGValue(dwColor)), static_cast<BYTE>(GetRValue(dwColor)));
		pNode->SetAttribute("textshadowcolorb", StringConvertor::WideToUtf8(szPropertyInf));
	}
